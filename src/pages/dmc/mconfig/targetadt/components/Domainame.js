...
import { Input, Modal, Space, Switch, Tag, Tooltip, message, Popconfirm, Card } from 'antd';
...
const Domainame = () => {
  ...
  const handleSave = () => {
    ...
    let data = {};
    data.module = module;
    let obj = formRef.current.getFieldsValue(['rule_id', 'state', 'risk', 'dns', 'rule_type', 'match_type', 'desc'])
  ...
  return (<>
    ...
    <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
      autoFocusFirstInput title={language('dmcoconfig.targetadt.domainname.modaltitle')}
      ...
      <ProFormText label={language('dmcoconfig.targetadt.dns')} name='dns' rules={[{
        required: true,
