...
const Keyword = () => {
  ...
  const module = 'keyword_file';
  const apiShowurl = '/cfg.php?controller=confPolicy&action=show';
  let rowkey = (record => record.rule_id);
  let searchVal = { queryVal: queryVal, queryType: 'fuzzy', module: 'keyword_file' };
  ...
  return (
    <>
      ...
      <ModalForm formRef={formRef} {...modalFormLayout} submitTimeout={2000}
        autoFocusFirstInput title={language('dmcoconfig.transmchk.keyword.modaltitle')}
        ...
        <ProFormText name='min_match_count' label={language('dmcoconfig.transmchk.min_match_num')} rules={[{
          required: true,
